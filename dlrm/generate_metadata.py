...
def calculate_cardinality_and_tokenizer_values(column):
    ...
    tokenizer_values = dict(zip(column.unique().tolist(), list(range(cardinality))))
    return cardinality, tokenizer_values

@click.command()
@click.option('--file_path', type=click.Path(exists=True), help='Path to the parquet file')
@click.option('--output_path', type=click.Path(), default='output.json', help='Path to the output JSON file')
